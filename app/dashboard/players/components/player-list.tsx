...
import { deletePlayer, updatePlayerStatus } from "../actions"; // Make sure to import deletePlayer
...
function StatusButton({ player, status }: { player: Player; status: "active" | "archived" }) {
  ...
  const handleAction = async (formData: FormData) => {
    ...
    if (result.error) {
      ...
      showToast(result.success ?? "Success", "success");
    }
  };
...
export function PlayerList({ players, onEdit, onDeleteSuccess, view }: PlayerListProps) {
  const { showToast } = useToast();
  ...
  const handleDelete = async (player: Player) => {
    if (window.confirm(`Are you sure you want to permanently delete ${player.first_name} ${player.last_name}? This cannot be undone.`)) {
      ...
      if (result.error) {
        ...
        showToast(result.success ?? "Success", "success");
        onDeleteSuccess(player.id); // This is now used
      }
    }
  };
  ...
  return (
    <div className="space-y-4">
      {players.map((player) => (
        <div key={player.id} className="p-4 bg-white border rounded-md flex items-center justify-between">
          <div>
