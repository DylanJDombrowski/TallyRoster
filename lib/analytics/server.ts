...
export async function getOrganizationAnalytics(
  ...
  const pageViewCounts = pageViewsData?.reduce((acc, row) => {
    const key = row.page_path;
    acc[key] = acc[key] || { path: row.page_path, title: row.page_title, views: 0 };
    acc[key].views++;
    return acc;
  ...
  const teamViewCounts = teamViewsData?.reduce((acc, row) => {
    const teamId = row.metadata?.teamId;
    const teamName = row.metadata?.teamName;
    if (teamId) {
      acc[teamId] = acc[teamId] || { teamId, teamName, views: 0 };
      acc[teamId].views++;
    }
    return acc;
  ...
  const playerViewCounts = playerViewsData?.reduce((acc, row) => {
    const playerId = row.metadata?.playerId;
    const playerName = row.metadata?.playerName;
    if (playerId) {
      acc[playerId] = acc[playerId] || { playerId, playerName, views: 0 };
      acc[playerId].views++;
    }
    return acc;
  ...
  // Get views by day
  const { data: dailyViewsData } = await baseQuery
    .select('created_at, session_id');

  const dailyStats = dailyViewsData?.reduce((acc, row) => {
